    ...
    create: function (callback) {
      var locations = paths.forEach(function (directory) {
        ...
        directory.tokens.forEach(function (token) {
          ...
          /* path.join.apply(object, array) allows us to use an array
           * as an argument to path instead of a comma delimited
           * list. Think of 'apply' as a form of message passing with
           * arguments.
           */
...
var cloneLibrary = function (library, callback) {
  ...
  templateNodes.forEach(function (templateNode) {
    var libraryFilePath     = path.join(library.path, templateNode);
    var destinationFilePath = path.join(library.destination, templateNode);
    fs.writeFileSync(
      destinationFilePath,
      fs.readFileSync(libraryFilePath)
    );
  });
